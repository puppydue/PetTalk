...
def tao_su_kien(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            event.save()
            messages.success(request, "🎉 Sự kiện đã được gửi và đang chờ phê duyệt.")
...
def chinh_sua_su_kien(request, event_id):
    """Trang chỉnh sửa sự kiện của người tạo"""
    ...
    return render(request, 'events/chinh_sua_su_kien.html', {'form': form, 'event': event})


# 3️⃣ Xoá sự kiện
@login_required
def xoa_su_kien(request, event_id):
    """Xoá sự kiện (chỉ người tạo mới được phép xoá)"""
    event = get_object_or_404(Event, id=event_id, creator=request.user)

    if request.method == 'POST':
        event.delete()
        messages.warning(request, f"🗑️ Sự kiện '{event.title}' đã bị xoá.")
        return redirect('danh_sach_su_kien')

    return render(request, 'events/xoa_su_kien.html', {'event': event})


# 4️⃣ Phê duyệt sự kiện (Moderator / Staff)
@user_passes_test(lambda u: u.is_staff)
def phe_duyet_su_kien(request):
    """Dành cho admin hoặc moderator phê duyệt sự kiện"""
    if request.method == 'POST':
        event_id = request.POST.get('approve') or request.POST.get('reject')
...
def dang_ky_tham_gia(request, event_id):
    ...
    if event.is_past():
        messages.warning(request, f"⏰ Sự kiện '{event.title}' đã kết thúc, bạn không thể đăng ký.")
        return redirect('danh_sach_su_kien')

    if request.method == 'POST':
        if not event.participants.filter(id=request.user.id).exists():
            event.participants.add(request.user)
            messages.success(request, f"🐾 Bạn đã đăng ký tham gia '{event.title}' thành công!")
        else:
            messages.info(request, f"⚠️ Bạn đã đăng ký sự kiện '{event.title}' trước đó rồi.")
        return redirect('danh_sach_su_kien')
...
def huy_dang_ky_tham_gia(request, event_id):
    ...
    if request.method == 'POST':
        if event.participants.filter(id=request.user.id).exists():
            ...
            messages.warning(request, f"❌ Bạn đã huỷ đăng ký tham gia '{event.title}'.")
        else:
            messages.error(request, f"⚠️ Bạn chưa đăng ký sự kiện '{event.title}' để huỷ.")
        return redirect('danh_sach_su_kien')
...
def danh_sach_su_kien(request):
    """Hiển thị danh sách sự kiện của người dùng và cộng đồng"""
...
def danh_sach_nguoi_dang_ky(request, event_id):
    """Hiển thị danh sách người tham gia một sự kiện"""
    event = get_object_or_404(Event, id=event_id)
